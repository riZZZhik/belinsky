...
class PhraseFinderTest(flask_unittest.ClientTestCase):
    ...
    def test_tokenizer(self, _: FlaskClient) -> None:
        ...
        response = [
            token.to_list()
            for token in self.comparer.tokenize("Мама обожает апельсины", "ru")
        ]
        correct_response = [
    ...
    def test_signup(self, client: FlaskClient) -> None:
        ...
        response = client.post(
            "/signup", json={"username": "unittester_1", "password": "test_password"}
        )
    ...
    def test_logout(self, client: FlaskClient) -> None:
        ...
        correct_response = {"result": "Successfully logged out.", "status": 200}
        self.assertEqual(response.json, correct_response)
    ...
    def test_find_phrase(self, client: FlaskClient) -> None:
        ...
        response = client.get(
            "/find-phrases",
            json={
                "text": "папа ненавидит апельсины",
                "phrases": ["ненавижу апельсины"],
            },
        )
        correct_response = {"result": {"ненавижу апельсины": [[5, 23]]}, "status": 200}
        self.assertEqual(response.json, correct_response)
    ...
    def test_find_phrase_translit(self, client: FlaskClient) -> None:
        ...
        correct_response = {"result": {"бананы": [[15, 20]]}, "status": 200}
        self.assertEqual(response.json, correct_response)
    ...
    def test_find_phrase_multiple_in_text(self, client: FlaskClient) -> None:
        ...
        correct_response = {"result": {"банан": [[0, 4], [17, 22]]}, "status": 200}
        self.assertEqual(response.json, correct_response)
    ...
    def test_find_phrase_hyphen(self, client: FlaskClient) -> None:
        ...
        correct_response = {"result": {"обожает любой": [[5, 21]]}, "status": 200}
        self.assertEqual(response.json, correct_response)

    def test_find_phrase_unknown_language(self, client: FlaskClient) -> None:
        """Test find phrase with unknown language."""
        response = client.get(
            "/find-phrases", json={"text": "Dies ist ein deutsch.", "phrases": []}
        )
        correct_response = {
